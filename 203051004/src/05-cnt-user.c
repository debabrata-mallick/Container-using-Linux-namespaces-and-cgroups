...
int configure_child_uid_gid_mapping(pid_t child_pid, int pipe_fd[])
{
    ...
    int uid_map = 0;
    int signal = -1;
    if (read(pipe_fd[0], &signal, sizeof(signal)) != sizeof(signal)) {
    ...
    if (write(pipe_fd[1], & (int) { result }, sizeof(int)) != sizeof(int)) {
        fprintf(stderr, "couldn't write: %m\n");
        return -1;
    }
...
int create_child_process(int flags, struct proc_info *config)
{
    ...
    if (configure_child_uid_gid_mapping(pid, config->pipe_fd)) {
...
finish_config:
    ...
    if (create_child_process(CLONE_NEWUSER | CLONE_NEWNS | CLONE_NEWPID | CLONE_NEWIPC | CLONE_NEWUTS, &config) == -1) {
